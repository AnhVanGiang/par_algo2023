...
//     // long Js_size = 0; // Size of the local Js vector
...
int main(int argc, char **argv) {
    ...
    // if (argc > 1) N = atol(argv[1]);1, 2,

    double *r = vecallocd(N);
    double *u = vecallocd(N);
    double p = 0.85;
    double *D = vecallocd(N);

    makeStochasticVector(u, N);

    CSRGraph graph = generateCSRGraph(N, 0.3);
    createDiagFromCSR(graph, N, D);

    long iter = solvePageRank(u, p, D, graph, N, r);
